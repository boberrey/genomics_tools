...
# output_filename <- "output/counts_matrix/tst.txt"
...
# Save a raw counts matrix:
write.table(cbind(as.character(peak_df$peak_name), overlap_matrix), file = "output/counts_matrix/raw_counts.txt",
...
if(normalization == 'housekeeping'){
  ...
  write.table(norm_peaks, file = paste(dirname(output_filename), "/", "housekeeping_peaks.bed", sep=""), 
              sep = '\t', col.names = TRUE, row.names = FALSE, quote = FALSE)
  # Also write the scale factors
  write.table(scale_factors, file = paste(dirname(output_filename), "/", "scale_factors.txt", sep=""),
