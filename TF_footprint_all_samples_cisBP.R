...
# Usage: 
# TF_footprint_all_samples.R <peak_file> <motif_file> <bam_dir> <output_dir> <tf_flank> <genome> <n_cores>
...
bam_dir <- args[3]
out_dir <- args[4]
tf_flank <- as.numeric(args[5]) # Usually 250
g <- args[6] # {'mm10', 'hg38'}
...
# Calculate each motif footprint for each sample:
get_footprints <- function(insertions, motifs, n_cores=5){
  ins_cov <- coverage(insertions)
  ...
  motif_footprints <- lapply(motifs, function(x) per_motif_footprint(ins_cov, x, motif_width))
...
footprint_file <- paste(out_dir, "/", "per_sample_footprints_cisBP.rds",sep="")
